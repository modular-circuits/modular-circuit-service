...
export const P = {
   ...
   */
  positional(name: string, typefn: TypeProcessor = T.any): PropertyDefinition {
    return {
   ...
   */
  collection(name: string, accept: string, typefn: TypeProcessor = T.any): PropertyDefinition {
    return {
      kind: Kind.item_list,
      name: name,
      accepts: [accept],
   ...
   */
  dict(name: string, accept: string, typefn: TypeProcessor = T.any): PropertyDefinition {
    return {
      kind: Kind.item_list,
      name: name,
      accepts: [accept],
...
export function parse_expr_partial(expr: string | List, ...defs: PropertyDefinition[]) {
  if (is_string(expr)) {
    log.info(`Parsing expression with ${expr.length} chars`)
    expr = listify(expr)
    if (expr.length == 1 && Array.isArray(expr[0])) {
