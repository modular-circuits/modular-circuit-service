...
import { P, type Parseable, T, parse_expr } from './parser'

export const is_global_pwr_symbol = (ref_name: string) => ref_name.startsWith('#PWR')
...
function mk_obj_deep_cp_omitting_properties_recursive(obj: any, ...props: string[]): any {
  if (typeof obj !== 'object' || obj === null) {
    return obj // Return non-object values as-is
  }

  // Check if it's an array and deep copy it accordingly
  if (Array.isArray(obj)) {
    return obj.map((item) => mk_obj_deep_cp_omitting_properties_recursive(item, ...props))
  }
  ...
  for (const key in obj) {
    if (!props.includes(key) && obj.hasOwnProperty(key)) {
      const val = obj[key]
      obj_cp[key] = mk_obj_deep_cp_omitting_properties_recursive(val, ...props)
    }
  }
...
export class KicadSch {
  ...
  constructor(
    ...
    Object.assign(
      ...
      parse_expr(
        ...
        P.collection('global_labels', 'global_label', T.item(GlobalLabel, this)),
        P.collection('hierarchical_labels', 'hierarchical_label', T.item(HierarchicalLabel, this)),
        // images
        P.mapped_collection('symbols', 'symbol', (p: SchematicSymbol) => p.uuid, T.item(SchematicSymbol, this)),
  ...
  *getChildren() {
  ...
  get labels() {
    return [...this.global_labels, ...this.net_labels, ...this.hierarchical_labels]
  }

  get sheet_pins() {
    const pins = new Map<string, HierarchicalSheetPin[]>()
    for (const it of this.sheets) pins.set(it.sheetfile!, it.hierarchicalSheetPins)
    return pins
  }
...
export class Wire {
  ...
  constructor(expr: Parseable) {
    ...
    Object.assign(
      this,
      parse_expr(expr, P.start('wire'), P.list('pts', T.vec2), P.item('stroke', Stroke), P.pair('uuid', T.string)),
    )
  }
}
...
export class Bus {
  ...
  constructor(expr: Parseable) {
    ...
    Object.assign(
      this,
      parse_expr(expr, P.start('bus'), P.list('pts', T.vec2), P.item('stroke', Stroke), P.pair('uuid', T.string)),
    )
  }
}
...
export class Arc extends GraphicItem {
  ...
  constructor(expr: Parseable, parent?: LibSymbol | SchematicSymbol) {
    ...
    const parsed = parse_expr(
      ...
      P.vec2('mid'),
      P.vec2('end'),
      P.object('radius', {}, P.start('radius'), P.vec2('at'), P.pair('length'), P.vec2('angles')),
...
export class Rectangle extends GraphicItem {
  ...
  constructor(expr: Parseable, parent?: LibSymbol | SchematicSymbol) {
    ...
    Object.assign(
      this,
      parse_expr(expr, P.start('rectangle'), P.vec2('start'), P.vec2('end'), ...GraphicItem.common_expr_defs),
    )
  }
}
...
export class HierarchicalLabel extends Label {
  ...
  constructor(expr?: Parseable) {
    ...
    if (expr) {
      Object.assign(
        this,
        parse_expr(expr, P.start('hierarchical_label'), ...Label.common_expr_defs, P.pair('shape', T.string)),
      )
    }
  }
}
...
export class LibSymbols {
  *getChildren() {
  ...
  constructor(
    ...
    Object.assign(
      this,
      parse_expr(expr, P.start('lib_symbols'), P.collection('symbols', 'symbol', T.item(LibSymbol, parent))),
    )
  ...
  constructor(
    ...
    Object.assign(
      ...
      parse_expr(
        ...
        P.object('pin_names', this.pin_names, P.pair('offset', T.number), P.atom('hide')),
        P.pair('exclude_from_sim', T.boolean),
        P.pair('in_bom', T.boolean),
        P.pair('on_board', T.boolean),
        P.mapped_collection('properties', 'property', (p: Property) => p.name, T.item(Property, this)),
    ...
    for (const pin of this.pins) {
      this.libPins.push(new LibSymbolPin(pin.number.text, `${this.name}:${pin.number}`, '', pin, pin.unit, this))
    }
  }
  ...
  constructor(
    ...
    Object.assign(
      ...
      parse_expr(
        ...
        P.object('name', this.name, P.positional('text', T.string), P.item('effects', Effects)),
        P.object('number', this.number, P.positional('text', T.string), P.item('effects', Effects)),
...
export class SchematicSymbol {
  ...
  get value() {
    return this.get_property_text('ALTIUM_VALUE') ?? this.get_property_text('Value') ?? ''
  }
  ...
  constructor(expr: Parseable) {
    Object.assign(
      ...
      parse_expr(
        ...
        P.mapped_collection('sheet_instances', 'path', (obj: SheetInstance) => obj.path, T.item(SheetInstance)),
      ),
    )
  }
  ...
  constructor(expr: Parseable) {
    Object.assign(
      ...
      parse_expr(
        ...
        P.mapped_collection('symbol_instances', 'path', (obj: SymbolInstance) => obj.path, T.item(SymbolInstance)),
      ),
    )
  }
...
export class SchematicSheet {
  *getChildren() {
  ...
  public get bbox() {
    return new BBox(this.at.position.x, this.at.position.y, this.size.x, this.size.y)
  }
  ...
  constructor(
    ...
    const parsed = parse_expr(
      ...
      P.pair('uuid', T.string),
      P.mapped_collection('properties', 'property', (prop: Property) => prop.name, T.item(Property, this)),
        ...
        P.collection(
          ...
          T.object(
            ...
            P.collection(
              'paths',
              'path',
              T.object(null, P.start('path'), P.positional('path'), P.pair('page', T.string)),
            ),
          ),
        ),
      ),
