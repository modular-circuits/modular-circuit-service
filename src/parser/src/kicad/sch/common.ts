...
export function expand_text_vars(text: string, resolveable: HasResolveTextVars | undefined): string {
  ...
  text = text.replaceAll(/(\$\{(.+?)\})/g, (_substring: string, all: string, name: string) => {
...
export class Stroke {
  width: number
  type: 'dash' | 'dot' | 'dash_dot' | 'dash_dot_dot' | 'solid' | 'default' = 'default'
  color?: Color
  ...
  constructor(expr: Parseable) {
    ...
    Object.assign(
      this,
      parse_expr(expr, P.start('stroke'), P.pair('width', T.number), P.pair('type', T.string), P.color()),
    )
  }
}
