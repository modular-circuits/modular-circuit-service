...
import type { Paper } from './common'
import { DRAWING_SHEET_STR } from './default_drawing_sheet'
...
export class DrawingSheet {
  ...
  static default() {
    ...
    return new DrawingSheet(DRAWING_SHEET_STR)
  }

  *items() {
    // Yield a rect to draw the page outline
    yield new Rect(
      `(rect (name "") (start ${-this.setup.left_margin} ${-this.setup.right_margin} ltcorner) (end ${-this.setup
        .right_margin} ${-this.setup.bottom_margin} rbcorner) (comment "page outline"))`,
      this,
    )
    yield* this.drawings
  }
...
export class DrawingSheetItem {
  ...
  protected static common_expr_defs = [
